    ...
    : item.mode === 'city'
      ...
      : item.mode === 'music'
        ...
        : item.mode === 'diagnosis'
          ...
          : item.mode === 'book'
            ? [hint('Автор', list(item.bookAuthors, 2)), hint('Год публикации', item.bookPublicationYear), hint('Страна', item.bookCountry), hint('Жанры', list(item.bookGenres, 3))]
          : item.mode === 'character'
            ? [hint('Эпоха', item.characterEra), hint('Источник', list(item.characterSourceTypes, 2)), hint('Роль', list(item.characterRoles, 2)), hint('Способности', list(item.characterAbilities, 2))]
          : item.mode === 'anime'
