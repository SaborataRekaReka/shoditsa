...
export const probeMusicSourceHealth = async (environment: Record<string, string>, fetchImpl: typeof fetch = fetch) => {
  ...
  const musicbrainz = async (): Promise<SourceHealth> => {
    try {
      const response = await request(fetchImpl, 'https://musicbrainz.org/ws/2/artist/?query=artist%3ARihanna&fmt=json', { headers: { Accept: 'application/json', 'User-Agent': userAgent } })
  ...
  const lastfm = async (): Promise<SourceHealth> => {
    ...
    try {
      const response = await request(fetchImpl, `https://ws.audioscrobbler.com/2.0/?${params}`)
      const payload = await json(response)
  ...
  const theaudiodb = async (): Promise<SourceHealth> => {
    ...
    try {
      const response = await request(fetchImpl, `https://www.theaudiodb.com/api/v1/json/${encodeURIComponent(environment.THEAUDIODB_API_KEY)}/search.php?s=Rihanna`)
      const payload = await json(response)
  ...
  const spotify = async (): Promise<SourceHealth> => {
    ...
    try {
      const tokenResponse = await request(fetchImpl, 'https://accounts.spotify.com/api/token', {
        method: 'POST',
        headers: {
      ...
      const searchResponse = await request(fetchImpl, 'https://api.spotify.com/v1/search?q=artist%3ARihanna&type=artist&limit=1', {
  ...
  return { sources, disabledSources: Object.entries(sources).filter(([, health]) => health.state !== 'ok').map(([source]) => source) }
}
