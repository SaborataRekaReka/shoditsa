...
const COMPLETION_MODE_FIELDS: Record<ContentMode, CompletionField[]> = {
  ...
  animal: [
    { label: 'Научное название', present: (payload) => hasTextValue(payload.scientificName) || hasTextValue(payload.titleOriginal) },
    { label: 'Класс', present: (payload) => hasTextValue(payload.taxonomicClass) },
    { label: 'Среда', present: (payload) => hasContentValue(payload.habitats) },
      ...
      void task.completion.then(async (job) => {
        await deps.db.insert(auditLog).values({
          ...
          after: {
            ...
            mediaUrls: job.items.filter((item) => item.storage === 'media').map((item) => item.url),
            storage: {
              media: job.items.filter((item) => item.storage === 'media').length,
              memory: job.items.filter((item) => item.storage === 'memory').length,
            },
            error: job.error,
            warning: job.warning,
          },
