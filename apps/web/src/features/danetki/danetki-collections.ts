...
const DEFINITIONS: Record<DanetkiCollectionSlug, DanetkiCollectionDefinition> = {
  'dlya-detey': {
    ...
    description: (count) => `Данетки для детей с ответами: ${count} понятных историй на логику для дома, школы и детской компании. Условия, вопросы и игра с ИИ без спойлеров.`,
    ...
    paragraphs: [
      ...
      'Подборка подходит для семейного вечера, классного часа и небольшой детской компании. В каждой истории есть условие, стартовые вопросы и полный ответ под спойлером; выбранную загадку можно запустить с ИИ-ведущим без раскрытия решения.',
    ],
    ...
    guideSteps: [
      ...
      'Когда версия объясняет все детали, команда сверяется с ответом или запускает ИИ-ведущего.',
    ],
  ...
  slozhnye: {
    ...
    description: (count) => `Сложные данетки с ответами: ${count} трудных историй для опытных игроков. Проверяйте версии, открывайте подсказки или расследуйте дело с ИИ.`,
    ...
    paragraphs: [
      ...
      'Эти истории лучше разгадывать вдвоём или командой: один игрок проверяет обстоятельства, другой ищет скрытое допущение, третий собирает общую версию. Если расследование остановилось, используйте стартовые вопросы или ИИ-ведущего.',
    ],
  ...
  legkie: {
    ...
    description: (count) => `Лёгкие данетки с ответами: ${count} коротких историй для знакомства с игрой. Найдите разгадку сами или задавайте вопросы ИИ-ведущему.`,
    ...
    paragraphs: [
      ...
      'Начните с условия, попробуйте назвать наиболее простое объяснение и не бойтесь уточнять очевидные детали. Ответ каждой истории спрятан под спойлером, а режим с ИИ позволяет пройти ту же данетку как настоящее расследование.',
    ],
  ...
  novye: {
    ...
    description: (count) => `Новые данетки с ответами: ${count} последних историй редакционной коллекции «Сходится!». Читайте условия и играйте с ИИ без спойлеров.`,
    ...
    paragraphs: [
      ...
      'Каждая новая история проходит ту же проверку структуры: понятное условие, полная разгадка, стартовые вопросы и факты для ИИ-ведущего. Подборка обновляется вместе с библиотекой и не требует отдельного ручного копирования страниц.',
    ],
    ...
    guideSteps: [
      ...
      'Проверяйте условие без ответа и переходите в игру с ИИ, если сюжет вам незнаком.',
