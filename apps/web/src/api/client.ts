...
export const api = {
  ...
  danetkiStart: (id: string, idempotencyKey: string) => request<GameResponse>(`${API_BASE}/danetki/sessions/${id}/start`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ idempotencyKey }), retries: 1 }),
  ...
  friendsRoomTerritoryAnswer: (id: string, duelId: string, optionId: string, idempotencyKey: string) => request<FriendsRoomResponse>(`${API_BASE}/friends/rooms/${encodeURIComponent(id)}/territory/answers`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ duelId, optionId }), retries: 1 }),
  friendsRoomTerritoryCapture: (id: string, cellId: string, idempotencyKey: string) => request<FriendsRoomResponse>(`${API_BASE}/friends/rooms/${encodeURIComponent(id)}/territory/captures`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ cellId }), retries: 1 }),
  friendsRoomTerritoryRematch: (id: string, idempotencyKey: string) => request<FriendsRoomResponse>(`${API_BASE}/friends/rooms/${encodeURIComponent(id)}/territory/rematch`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: '{}', retries: 1 }),
  ...
  friendsRoomContinue: (id: string, idempotencyKey: string) => request<FriendsRoomResponse>(`${API_BASE}/friends/rooms/${encodeURIComponent(id)}/continue`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ idempotencyKey }), retries: 1 }),
  ...
  freePlay: (mode: ContentMode, difficulty: ApiDifficultyKey | null, key: string, sourceSessionId?: string) => request<FreePlayResponse>(`${API_BASE}/economy/free-play/start`, { method: 'POST', headers: { 'Idempotency-Key': key }, body: JSON.stringify({ mode, difficulty, sourceSessionId }), retries: 1, maxRateLimitRetryMs: 10_000 }),
