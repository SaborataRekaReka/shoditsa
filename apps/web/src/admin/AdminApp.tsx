...
const MODES: Array<{ value: ContentMode; label: string }> = [...CONTENT_MODE_IDS]
  .sort((left, right) => GAME_MODE_MANIFEST[left].dailyOrder - GAME_MODE_MANIFEST[right].dailyOrder)
  .map((value) => ({ value, label: GAME_MODE_MANIFEST[value].label }))
...
function ContentRevisionControl({ activeRevision, navigate, notify }: {
  ...
  const buildRelease = useMutation({
    mutationFn: async () => {
      ...
      if (!preview || !confirm(`Создать безопасную ревизию из текущего релиза?\n\nОбновится: ${preview.updated}\nДобавится: ${preview.added}\nСохранится только из БД: ${preview.preserved}\nКонфликтов режима: ${preview.conflicted}\nУдалится: 0\n\nКарточки с конфликтом режима сохранятся из активной БД. Активный контент не изменится до отдельной активации.`)) throw new Error('Действие отменено')
  ...
  return <section className="admin-panel admin-acquisition">
    ...
    {data && <>
      ...
      {data.diagnosisRecommendations && <section className="admin-acquisition__danetki">
        ...
        <p>Связь перехода с завершённой игрой обновлена 4 сентября. Проверка 7 сентября — техническая; чистая неделя 5–11 сентября доступна 12 сентября. Неделя 1–7 сентября смешивает старые и новые события. Цель по числу стартов применима только к семидневному окну.</p>
      </section>}
  ...
  return (
    <>
      <PageHead
        ...
        description={scopedMode ? `Поиск, проверка и публикация карточек категории «${MODE_LABEL[scopedMode]}».` : "Поиск, проверка и публикация всех игровых библиотек."}
        actions={
          <>
...
function EconomyPage({ notify }: { notify: (tone: Notice['tone'], text: string) => void }) {
  ...
  const metrics = useQuery({ queryKey: ['admin', 'economy', days], queryFn: () => adminApi.economyOverview(days) })
  const promos = useQuery({ queryKey: ['admin', 'promos'], queryFn: adminApi.promos })
  const [creating, setCreating] = useState(false)
  ...
  const ratioOk = summary?.earnedSpentRatio != null && summary.earnedSpentRatio >= .9 && summary.earnedSpentRatio <= 1.1
  const shortageOk = Boolean(summary && summary.shortageUserRate >= .2 && summary.shortageUserRate <= .35)
  ...
  return <>
    ...
    {metrics.isLoading ? <Loading /> : metrics.error ? <ErrorState error={metrics.error} /> : summary && overview && <>
      <div className="admin-economy-metrics">
        <article><span><Ticket /></span><small>Заработано / потрачено</small><strong>{summary.ticketsEarned} / {summary.ticketsSpent}</strong><em>ratio {summary.earnedSpentRatio?.toFixed(2) ?? '—'}</em></article>
        <article><span><AlertTriangle /></span><small>Столкнулись с нехваткой</small><strong>{percent(summary.shortageUserRate)}</strong><em>{summary.shortageUsers} из {summary.activeUsers} · {summary.shortageClubConversions} → Клуб</em></article>
        <article><span><Activity /></span><small>Уход после нехватки</small><strong>{percent(summary.shortageExitRate)}</strong><em>proxy: нет события 15 минут</em></article>
        <article><span><CircleGauge /></span><small>Баланс P50 / P90</small><strong>{Math.round(summary.balanceP50)} / {Math.round(summary.balanceP90)}</strong><em>билетов</em></article>
        <article><span><Bot /></span><small>Данетки P50 / P95</small><strong>{Math.round(summary.danetkiTokensP50)} / {Math.round(summary.danetkiTokensP95)}</strong><em>AI-токенов · {summary.danetkiRoomsMeasured} комнат</em></article>
        <article><span><CircleDollarSign /></span><small>Выручка / плательщика</small><strong>{Math.round(summary.revenueMinor / 100).toLocaleString('ru-RU')} ₽ / {Math.round(summary.revenuePerPayingUserMinor / 100).toLocaleString('ru-RU')} ₽</strong><em>{summary.repeatBuyers} повторных · {summary.refunds} возвратов</em></article>
      </div>
      <section className="admin-economy-targets">
        <article><Status value={ratioOk ? 'active' : 'warning'} /><span><strong>Earned / spent: 0,9–1,1</strong><small>Фактически {summary.earnedSpentRatio?.toFixed(2) ?? 'нет расходов'}</small></span></article>
        <article><Status value={shortageOk ? 'active' : 'warning'} /><span><strong>Нехватка: 20–35%</strong><small>Фактически {percent(summary.shortageUserRate)}</small></span></article>
        <article><Status value={exitsOk ? 'active' : 'warning'} /><span><strong>Уход после нехватки: ≤15%</strong><small>Фактически {percent(summary.shortageExitRate)}</small></span></article>
        <article><Status value={balancesOk ? 'active' : 'warning'} /><span><strong>P50 60–180, P90 &lt;700</strong><small>Фактически {Math.round(summary.balanceP50)} / {Math.round(summary.balanceP90)}</small></span></article>
      </section>
      <div className="admin-economy-observability">
        <section className="admin-panel"><header><div><span>Ticket sinks</span><h2>Расходы по типам</h2></div></header><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Причина</th><th>Билетов</th><th>Операций</th></tr></thead><tbody>{overview.spendByReason.map((entry) => <tr key={entry.reason}><td><strong>{entry.reason}</strong></td><td>{entry.amount}</td><td>{entry.operations}</td></tr>)}</tbody></table></div></section>
        <section className="admin-panel"><header><div><span>Rules audit</span><h2>Версии правил</h2></div></header><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Версия</th><th>Операций</th><th>Earn / spend</th></tr></thead><tbody>{overview.ruleVersions.map((entry) => <tr key={entry.rulesVersion}><td><strong>v{entry.rulesVersion}</strong></td><td>{entry.operations}</td><td>{entry.earned} / {entry.spent}</td></tr>)}</tbody></table></div><div className="admin-balance-buckets">{overview.balanceBuckets.map((entry) => <span key={entry.bucket}><strong>{entry.users}</strong><small>{entry.bucket} билетов</small></span>)}</div></section>
      </div>
      <div className="admin-economy-observability">
        <section className="admin-panel"><header><div><span>Runtime rules</span><h2>{overview.activeRule ? `Активная версия v${overview.activeRule.version}` : 'Активная версия не определена'}</h2></div><Status value={overview.activeRule ? 'active' : 'critical'} /></header>{overview.activeRule ? <><p>Действует с {formatDate(overview.activeRule.effectiveAt)}</p><pre>{JSON.stringify(overview.activeRule.rules, null, 2)}</pre></> : <p role="alert">Диагностика: {overview.activeRuleDiagnostic}</p>}</section>
        <section className="admin-panel"><header><div><span>Economy v4 usage</span><h2>Ключевые действия</h2></div></header><div className="admin-balance-buckets"><span><strong>{summary.friendsRoomContinuations}</strong><small>продолжений Friends Room</small></span><span><strong>{summary.clubDanetkiRooms}</strong><small>клубных Данеток</small></span><span><strong>{summary.ticketBundlePurchases}</strong><small>покупок ticket bundles</small></span></div></section>
      </div>
    </>}
    <div className="admin-economy-grid"><section className="admin-panel"><header><div><span>Промокоды</span><h2>Активные и завершённые</h2></div></header><div className="admin-promo-list">{promos.data?.items.map((raw) => { const entry = record(raw); const promo = record(entry.promo); return <article key={String(promo.id)}><span className="admin-list-icon"><Tags /></span><div><strong>{title(promo.title)}</strong><small>{promo.enabled ? 'Активен' : 'Отключён'} · {String(entry.redemptions ?? 0)} применений</small><p>{String(record(promo.rewardValue).amount ?? promo.rewardValue)} билетов · лимит {String(promo.perUserLimit)}/польз.</p></div><Status value={promo.enabled ? 'active' : 'blocked'} /></article> })}</div></section><section className="admin-panel admin-ledger-explainer"><header><div><span>Гарантия</span><h2>Ledger не редактируется</h2></div></header><CircleDollarSign /><h3>Каждая корректировка — отдельная операция</h3><p>Баланс меняется только через audited endpoint с обязательной причиной и idempotency key. Старые операции нельзя удалить или переписать.</p><a href="/admin/users">Найти пользователя для корректировки <ChevronRight /></a></section></div>
    {creating && <div className="admin-modal-backdrop"><div className="admin-modal"><header><div><span>Credential показывается один раз</span><h2>Новый промокод</h2></div><button onClick={() => setCreating(false)}><X /></button></header><div className="admin-modal__body"><label className="admin-field"><span>Сырой код</span><input value={code} onChange={(event) => setCode(event.target.value)} placeholder="SHODITSA-2026" /></label><label className="admin-field"><span>Название</span><input value={promoTitle} onChange={(event) => setPromoTitle(event.target.value)} placeholder="Летний подарок" /></label><label className="admin-field"><span>Билетов</span><input type="number" min="1" value={reward} onChange={(event) => setReward(Number(event.target.value))} /></label><div className="admin-warning"><AlertTriangle />После создания БД хранит только HMAC hash. Потерянный код восстановить нельзя.</div></div><footer><button className="admin-btn admin-btn--secondary" onClick={() => setCreating(false)}>Отмена</button><button className="admin-btn admin-btn--primary" onClick={() => create.mutate()} disabled={!code || !promoTitle || create.isPending}>Создать код</button></footer></div></div>}
  </>
}
...
function IntegrationsPage({ notify }: { notify: (tone: Notice['tone'], text: string) => void }) {
  ...
  const startPortraitTest = useMutation({
    mutationFn: (selection?: { portraitId?: 'sherlock-holmes'; portraitBatch?: 'character-expansion-50' | 'character-expansion-330' }) => adminApi.startOpenAiPortraitTest(selection),
    onSuccess: (job) => { setPortraitTestId(job.id); notify('info', `Генерация ${job.count === 1 ? 'портрета' : `${job.count} портретов`} запущена`) },
    onError: (error) => notify('error', errorText(error)),
  })
  const save = useMutation({
  ...
  return <><PageHead eyebrow="Зашифрованное хранилище" title="API-интеграции" description="Ключи платёжных, музыкальных и контентных сервисов в одном месте. Исходные секреты никогда не возвращаются в браузер." actions={<button className="admin-btn admin-btn--secondary" onClick={() => void integrations.refetch()}><RefreshCw />Обновить</button>} />
    ...
    <section className="admin-panel admin-portrait-test"><header><div><span>GPT Image 2 · low</span><h2>Тест портретов персонажей</h2></div><Status value={portraitTest.data?.status ?? 'neutral'}>{portraitTest.data?.status === 'completed' ? 'Готово' : portraitTest.data?.status === 'failed' ? 'Ошибка' : portraitTestId ? 'Генерация' : 'Не запускался'}</Status></header><p>Фирменный стиль «Сходится», 1024×1536. Один Холмс — около $0.005, тестовая серия из пяти — около $0.025, расширение из 50 — около $0.25, расширение из 330 — около $1.65 за вывод; ключ остаётся на сервере.</p><div className="admin-actions"><button className="admin-btn admin-btn--primary" disabled={startPortraitTest.isPending || portraitTest.data?.status === 'running'} onClick={() => { if (confirm('Сгенерировать только Шерлока Холмса GPT Image 2 в качестве low? Оценка стоимости вывода: $0.005.')) startPortraitTest.mutate({ portraitId: 'sherlock-holmes' }) }}><WandSparkles />{startPortraitTest.isPending || portraitTest.data?.status === 'running' ? 'Генерация…' : 'Проверить Холмса · $0.005'}</button><button className="admin-btn admin-btn--secondary" disabled={startPortraitTest.isPending || portraitTest.data?.status === 'running'} onClick={() => { if (confirm('Сгенерировать 5 тестовых портретов GPT Image 2 в качестве low? Оценка стоимости вывода: $0.025.')) startPortraitTest.mutate(undefined) }}>Сгенерировать тестовые 5</button><button className="admin-btn admin-btn--secondary" disabled={startPortraitTest.isPending || portraitTest.data?.status === 'running'} onClick={() => { if (confirm('Сгенерировать 50 новых портретов персонажей GPT Image 2 в качестве low? Оценка стоимости вывода: $0.25 без возможных повторов.')) startPortraitTest.mutate({ portraitBatch: 'character-expansion-50' }) }}>Сгенерировать новые 50 · $0.25</button><button className="admin-btn admin-btn--secondary" disabled={startPortraitTest.isPending || portraitTest.data?.status === 'running'} onClick={() => { if (confirm('Сгенерировать 330 новых портретов персонажей GPT Image 2 в качестве low? Оценка стоимости вывода: $1.65 без возможных повторов.')) startPortraitTest.mutate({ portraitBatch: 'character-expansion-330' }) }}>Сгенерировать новые 330 · $1.65</button></div>{portraitTest.data?.error && <p className="admin-portrait-test__error">{portraitTest.data.error}</p>}{portraitTest.data?.warning && <p>{portraitTest.data.warning}</p>}{portraitTest.data?.items.length ? <div className="admin-portrait-test__grid">{portraitTest.data.items.map((item) => <a key={item.id} href={item.url} target="_blank" rel="noreferrer"><img src={item.url} alt={item.title} /><span><strong>{item.title}</strong><small>{item.width}×{item.height} · {Math.ceil(item.bytes / 1024)} КБ · {item.storage === 'media' ? 'media' : 'память 1 ч.'}</small></span></a>)}</div> : null}</section>
...
export default function AdminApp() {
  ...
  if (me.isLoading || (me.data?.user.role === 'admin' && access.isLoading)) return <div className="admin-gate"><div className="admin-gate__brand"><img src={adminLogoUrl} alt="Сходится!" /><LoaderCircle /></div><p>Проверяем административный доступ…</p></div>
