...
export const adminApi = {
  ...
  pipelineEstimate: (pipeline: 'music' | 'movie', body: Record<string, unknown>) => request<Record<string, unknown>>(`/admin/pipelines/${pipeline}/estimate`, { method: 'POST', body: json(body) }),
  pipelineManualPreview: (pipeline: 'music' | 'movie', items: Array<Record<string, unknown>>) => request<{ items: Array<Record<string, unknown>>; summary: Record<string, number> }>(`/admin/pipelines/${pipeline}/manual/preview`, { method: 'POST', body: json(pipeline === 'music' ? { artists: items } : { movies: items }) }),
  startPipeline: (pipeline: 'music' | 'movie', body: Record<string, unknown>) => request<{ runId: string; jobId: string }>(`/admin/pipelines/${pipeline}/runs`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey() }, body: json({ ...body, confirmation: true }) }),
