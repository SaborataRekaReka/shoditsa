import type {
  AdminAcquisitionFunnelPeriod, AdminAcquisitionFunnelResponse, AdminCampaignFunnelResponse, AdminCampaignPeriod, AdminContentItemsResponse, AdminContentTag, AdminDashboardResponse, AdminTimelineEvent, AdminUserListItem, AdminWorkspaceSummary,
...
export const adminApi = {
  ...
  campaignFunnel: (days: AdminCampaignPeriod) => request<AdminCampaignFunnelResponse>(`/admin/campaign-funnel?days=${days}`),
  ...
  uploadBuilderMedia: async (file: File, purpose: 'posterUrl' | 'headerUrl') => request<{ url: string; width: number; height: number; bytes: number }>('/admin/content/builder/media', { method: 'POST', body: json({ fileName: file.name, contentType: file.type, base64: await fileBase64(file), purpose }), timeoutMs: 60_000 }),
  ...
  pipelineBulkDecision: (runId: string, body: { itemIds: string[]; approved: boolean; note?: string }) => request<{ success: number; failed: number; approved: boolean; itemIds: string[] }>(`/admin/pipeline-runs/${runId}/items/decisions`, { method: 'PATCH', body: json(body), timeoutMs: 120_000 }),
  ...
  retryUnresolvedPipelineItems: (runId: string) => request<{ job: Record<string, unknown>; unresolvedCount: number }>(`/admin/pipeline-runs/${runId}/retry-unresolved`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey() }, body: '{}' }),
  reprocessFactcheckResults: (runId: string) => request<{ job: Record<string, unknown>; unresolvedCount: number }>(`/admin/pipeline-runs/${runId}/reprocess-results`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey() }, body: '{}' }),
  ...
  danetkiSession: (id: string) => request<Record<string, unknown>>(`/admin/danetki/sessions/${id}`),
  retryDanetkiAi: (id: string) => request<Record<string, unknown>>(`/admin/danetki/sessions/${id}/retry-ai`, { method: 'POST', body: '{}' }),
  closeDanetkiSession: (id: string, reason: string) => request<Record<string, unknown>>(`/admin/danetki/sessions/${id}/force-close`, { method: 'POST', body: json({ reason }) }),
  testDanetkiAi: (question: string) => request<Record<string, unknown>>('/admin/danetki/ai-test', { method: 'POST', body: json({ question }), timeoutMs: 70_000 }),
  ...
  patchCommerceProduct: (id: string, body: Record<string, unknown>) => request<Record<string, unknown>>(`/admin/commerce/products/${encodeURIComponent(id)}`, { method: 'PATCH', body: json(body) }),
  commerceOrders: () => request<{ items: Array<Record<string, unknown>> }>('/admin/commerce/orders?limit=100'),
  commerceEntitlements: () => request<{ items: Array<Record<string, unknown>> }>('/admin/commerce/entitlements?limit=100'),
  grantCommerceEntitlement: (body: Record<string, unknown>) => request<Record<string, unknown>>('/admin/commerce/entitlements/grant', { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey() }, body: json(body) }),
  revokeCommerceEntitlement: (id: string, reason: string) => request<Record<string, unknown>>(`/admin/commerce/entitlements/${id}/revoke`, { method: 'POST', body: json({ reason }) }),
  privateGameOrders: () => request<{ items: Array<Record<string, unknown>> }>('/admin/private-game-orders'),
  patchPrivateGameOrder: (id: string, body: Record<string, unknown>) => request<Record<string, unknown>>(`/admin/private-game-orders/${id}`, { method: 'PATCH', body: json(body) }),
