...
export const adminApi = {
  ...
  pipelineEstimate: (pipeline: 'music' | 'movie' | 'anime', body: Record<string, unknown>) => request<Record<string, unknown>>(`/admin/pipelines/${pipeline}/estimate`, { method: 'POST', body: json(body) }),
  ...
  startPipeline: (pipeline: 'music' | 'movie' | 'anime', body: Record<string, unknown>) => request<{ runId: string; jobId: string }>(`/admin/pipelines/${pipeline}/runs`, { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey() }, body: json({ ...body, confirmation: true }) }),
