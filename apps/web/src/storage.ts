...
export const clearLocalProgressData = () => {
  ...
  for (const key of keys) {
    if (
      ...
      || key.startsWith(DAILY_MILESTONES_PREFIX)
      || key.startsWith(FREE_PLAY_USAGE_PREFIX)
      || key === ATTENDANCE_STATS_KEY
      || key === WALLET_KEY
      || key === TICKET_LEDGER_KEY
      || key === PROMO_USAGE_KEY
      || key === PERIOD_UNLOCKS_KEY
    ) {
