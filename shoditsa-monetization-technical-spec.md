...
Существует `dtf-games-promo-30-v1`, `promoPrompt`, `progressiveHints` и загрузка promo pack в game service. Сейчас `packId` разрешён только администратору и запускается через обычную daily-сессию.
...
Импортировать существующий `dtf-games-promo-30-v1` в эти таблицы через идемпотентный script/import step. Исходный JSON можно сохранить как staging source.
