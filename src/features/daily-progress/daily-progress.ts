...
const MODE_PLAY_LABEL: Record<TitleMode, string> = {
  movie: 'кино',
  series: 'сериалы',
  anime: 'аниме',
  game: 'игры',
...
export const buildDailyHubState = (
  ...
  const activeGames = games
    .filter((game) => game.status === 'playing' && game.attempts.length > 0)
    .sort(newestFirst)
  ...
  const recommendedMode = unfinishedModes.includes(preferredMode)
    ? preferredMode
    : unfinishedModes[0] ?? preferredMode
  ...
  return {
    ...
    recommendedMode,
    primaryLabel: activeGame
      ? `Продолжить ${MODE_ACCUSATIVE[activeGame.mode]}`
      : completedCount === 0
        ? 'Играть сейчас'
        : completedCount < DAILY_MODE_ORDER.length
          ? `Играть в ${MODE_PLAY_LABEL[recommendedMode]}`
          : 'Ещё одна загадка',
    primaryMeta: activeGame
      ? `${activeGame.attempts.length} из 10 попыток`
      : completedCount > 0 && completedCount < DAILY_MODE_ORDER.length
        ? 'Следующая игра дня'
        : null,
    punchesCaption: activeGame
      ? `${DAILY_MODE_LABELS[activeGame.mode]} в процессе`
      : completedCount === 0
        ? 'Выберите первую игру'
        : completedCount < DAILY_MODE_ORDER.length
          ? 'Выберите следующую игру'
          : 'Все игры дня завершены',
