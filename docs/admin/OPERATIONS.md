...
Workspace не меняет active content. Build создаёт полную immutable revision и останавливается при validation errors, конфликте base revision, повторном checksum или подозрительном падении числа карточек. Activation — единственный шаг, меняющий runtime-контент.

Перед опасной публикацией убедитесь, что свежи PostgreSQL backup и media backup по runbook из `docs/backend`. Для rollback выберите предыдущую revision в «Система → Ревизии» и активируйте её; endpoint принимает `ready` или уже `active`, поэтому retired revision сначала должна быть переведена в разрешённое состояние по согласованной DB-операции/отдельному rollback runbook. Не изменяйте payload активной revision вручную.
