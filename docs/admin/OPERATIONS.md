...
Перед музыкальной партией worker проверяет MusicBrainz, Last.fm, TheAudioDB и Spotify. Недоступные источники временно пропускаются, а карточка собирается из оставшихся структурированных источников и AI fact-check. Для этих четырёх провайдеров можно сохранить `MUSIC_OUTBOUND_PROXY_URL` в разделе «API-интеграции»: значение шифруется, поддерживаются доверенные HTTP/HTTPS proxy с авторизацией, а Wikidata, Deezer и iTunes остаются прямыми резервами. Некорректная proxy-настройка закрывает внешний маршрут без утечки credentials. Счётчики запуска должны совпадать с количеством `pipeline_run_items`; каждая server-партия связывается с результатами через отдельный manifest, а не через общий `mtime` файлов.
