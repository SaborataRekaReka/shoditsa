...
const source = process.env.ENRICHMENT_DATA_ROOT
  ? path.join(path.resolve(process.env.ENRICHMENT_DATA_ROOT), 'movie', 'discovery', 'discovered-candidates.json')
  : 'data/enrichment-agent/movie/discovery/discovered-candidates.json'
...
run(['scripts/enrichment-agent/run.mjs', 'movie', 'discover', ...forwarded])
run([
  'scripts/enrichment-agent/run.mjs', 'movie', 'run', `--source=${source}`,
