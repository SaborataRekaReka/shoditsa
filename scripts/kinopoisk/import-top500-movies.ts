...
const loadTop500Ids = async () => {
  const top500Path = resolve(root, 'data', 'reference', 'kinopoisk-top500-ids.json')
  if (!existsSync(top500Path)) throw new Error('Файл data/reference/kinopoisk-top500-ids.json не найден. Сначала подготовьте список ID из Top-500.')
  const raw = await readFile(top500Path, 'utf8')
  const parsed = JSON.parse(raw)
  if (!Array.isArray(parsed)) throw new Error('data/reference/kinopoisk-top500-ids.json должен быть массивом чисел')
  const ids = parsed.map((value) => Number(value)).filter((value) => Number.isInteger(value) && value > 0)
  const unique = [...new Set(ids)]
  if (unique.length !== 500) {
    throw new Error(`Ожидалось 500 уникальных id в data/reference/kinopoisk-top500-ids.json, получено ${unique.length}`)
  }
  return unique
}
