...
import { DANETKI_DISCOVERY_LINKS } from '../../apps/web/src/app/danetki-discovery-links'
import {
...
const renderGameLinks = (currentMode?: GameSeoContent['mode']) => PUBLIC_GAME_LINKS
  ...
  .join('')
const renderDanetkiDiscoveryLinks = () => DANETKI_DISCOVERY_LINKS
...
const renderDanetkiRecommendationsFallback = () => `<section class="danetki-recommendations" aria-labelledby="danetki-recommendations-static"><header class="danetki-recommendations__header"><span>Маршрут продолжается</span><h2 id="danetki-recommendations-static">Ещё три игры, где важна хорошая версия</h2><p>Выберите следующий короткий раунд — правила объяснятся по ходу игры.</p></header><div class="danetki-recommendations__grid"><a class="danetki-recommendation-card is-primary" href="/games/diagnosis"><span class="danetki-recommendation-card__copy"><small>Стройте версию по уликам</small><strong>Угадайте диагноз</strong><span>Сопоставляйте симптомы, систему органов и диагностические признаки.</span></span><span class="danetki-recommendation-card__action">Играть ↗</span></a><a class="danetki-recommendation-card" href="/games/character"><span class="danetki-recommendation-card__copy"><small>Новое досье каждый день</small><strong>Угадайте персонажа</strong><span>Ищите героя по происхождению, роли, эпохе и способностям.</span></span><span class="danetki-recommendation-card__action">Играть ↗</span></a><a class="danetki-recommendation-card" href="/games/animal"><span class="danetki-recommendation-card__copy"><small>Наблюдение вместо допроса</small><strong>Угадайте животное</strong><span>Сверяйте среду обитания, строение, питание и ареал.</span></span><span class="danetki-recommendation-card__action">Играть ↗</span></a></div></section>`
...
const renderDanetkiStoryFallback = (item: DanetkiCatalogItem) => {
  ...
  return `<main class="danetki-catalog-main danetki-story-main"><nav class="danetki-breadcrumbs" aria-label="Хлебные крошки"><a href="/">Сходится!</a><span>/</span><a href="/danetki">Данетки</a><span>/</span><span>${escapeHtml(item.titleRu)}</span></nav><article class="danetki-story"><header class="danetki-story__header"><div><span class="danetki-catalog-eyebrow">Данетка с ответом</span><h1>${escapeHtml(heading)}</h1>${aliases}<div class="danetki-story__tags"><span>${escapeHtml(danetkiDifficultyLabel(item.difficulty))}</span>${item.genres.map((genre) => `<span>${escapeHtml(genre)}</span>`).join('')}</div></div></header><section class="danetki-story__condition"><span>Условие</span><h2>Что произошло?</h2><p>${escapeHtml(item.condition)}</p></section><section class="danetki-story__questions"><span>Если не знаете, с чего начать</span><h2>Стартовые вопросы</h2><ul>${item.starterQuestions.map((question) => `<li><span>${escapeHtml(question)}</span></li>`).join('')}</ul></section><aside class="danetki-story__play"><div><span>Ответ пока скрыт</span><h2>Сначала попробуйте сыграть</h2><p>Ведущий отвечает «да», «нет» или «неважно» и помогает шаг за шагом восстановить историю.</p></div><a class="ui-button ui-button--primary" href="/games/danetki?from=story&amp;story=${escapeHtml(danetkiStoryPath(item).split('/').at(-1) ?? '')}#game">Играть с ведущим</a></aside><details class="danetki-story__answer"><summary><span><strong>Все равно показать</strong><small>Откроется готовая разгадка</small></span><span aria-hidden="true">+</span></summary><div><span>Авторская разгадка</span><p>${escapeHtml(item.solution)}</p></div></details></article><section class="danetki-story-related"><div class="danetki-catalog-section-head"><div><span>Следующие дела</span><h2>Похожие данетки</h2></div><a href="/danetki">Все истории →</a></div><div class="danetki-catalog-grid">${related.map(renderDanetkiCard).join('')}</div></section></main>`
}
