...
docker compose -f "$ROOT/app/compose.production.yml" --profile migrate run --rm --no-deps --entrypoint node migrate apps/api/dist/content-import-kpop-special.js --apply --activate --actor-id="$ADMIN_USER_ID" --report=/tmp/kpop-artists-admin-v1-import-report.json
