...
When the user asks to improve SEO, organic traffic, search conversion, or to act on Google Search Console, Yandex Webmaster, Metrika, or Wordstat findings, read [docs/SEO_GROWTH_EXECUTION_PLAN.md](docs/SEO_GROWTH_EXECUTION_PLAN.md) before editing code. Refresh the evidence with the `$shoditsa-traffic-audit` skill, respect the document's sample-size gates and no-touch list, and do not call a change successful until its browser and post-release measurement gates are satisfied.
